...
> 使用 **MediaPipe Holistic** 進行全身姿勢與左右手部關鍵點偵測，結合手勢辨識（掌心揮動、食指點選、👌 OK 手勢）實現語言切換與計數歸零。介面採用深色玻璃磨砂設計，支援中英雙語切換，適合桌機與行動裝置。
...
| **手勢辨識** | <ul><li>**掌心揮動**（水平滑動）：切換語言（中文/English）</li><li>**食指點選**（食指伸直，其他手指彎曲）：切換語言（同掌心揮動）</li><li>**👌 OK 手勢**（母指與食指尖端靠近，其他三指伸直）：計數歸零並以語音報出 “0”（需維持 2 秒）</li></ul> |
...
| **響應式佈局** | 畫面會依裝置寬度自動調整：寬度 ≥ 900px 時採用雙欄（左側畫布，右側資訊欄）；較小螢幕（包括直屏與橫屏）則為單欄垂直堆疊。 |
...
3. **使用方式** 
   ...
   - **手勢操作**： 
     ...
     - 👌 OK 手勢（母指與食指尖端靠近，其他三指伸直）→ 計數歸零，語音報出「0」（需維持 2 秒），並顯示對應 toast。 
...
- **👌 OK 手勢**：母指與食指尖端靠近（距離 < 0.05），其他三指伸直，觸發計數歸零（需維持 2 秒）。
- 若想改為其他功能（例如調節音量、切換背景顏色），只需在對應的 `if (isOK) { … }`、`if (isPoint) { … }` 或 swipe 分支內替換為你想執行的程式碼。
